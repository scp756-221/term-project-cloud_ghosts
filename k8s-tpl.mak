...
loader: dynamodb-init $(LOG_DIR)/loader.repo.log cluster/loader.yaml
	...
	tools/build-configmap.sh gatling/resources/bestseller.csv cluster/bestseller-header.yaml | kubectl -n $(APP_NS) apply -f -
	tools/build-configmap.sh gatling/resources/reader.csv cluster/reader-header.yaml | kubectl -n $(APP_NS) apply -f -
...
$(LOG_DIR)/s3.repo.log: s3/Dockerfile s3/app.py s3/requirements.txt
	make -f k8s.mak --no-print-directory registry-login
	$(DK) build $(ARCH) -t $(CREG)/$(REGID)/cmpt756s3:$(APP_VER_TAG) s2 | tee $(LOG_DIR)/s3.img.log
